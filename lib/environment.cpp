...
void Environment::initializeFloorplane(
    const FloorplaneClutterSettings &floorplane_settings) noexcept {
    if (!floorplane_settings.enable_static_floorplane || floorplane_settings.range_m <= 0.0f) {
        clearFloorplane();
        return;
    }

    const float lambda_m = Constants::kSpeedOfLightMps / radar_settings_.carrier_hz;
    const float chirp_duration_s = static_cast<float>(problem::RadarSettings::kRadarBlockSize) /
                                   radar_settings_.sample_rate_hz;
    const float chirp_slope_hz_per_s = radar_settings_.bandwidth_hz / chirp_duration_s;
    const float beat_frequency_hz =
        2.0f * chirp_slope_hz_per_s * floorplane_settings.range_m / Constants::kSpeedOfLightMps;
    ...
    const float base_phase_rad = -4.0f * Constants::kPi * floorplane_settings.range_m / lambda_m +
